...
def get_rank(s):
    variants = sorted(set (permutations(s)))
    return variants.index(tuple(s))+1

print(get_rank(input()))

'''
from itertools import permutations

word = input()
perm = permutations(word) # perm is an iterator object
myset = set(perm)

sortedmyset = sorted(myset)

newlist = []
for i in sortedmyset:
    newlist.append(''.join(i))

index = newlist.index(word) + 1
print(index)
